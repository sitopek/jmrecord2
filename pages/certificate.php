...
<body>
 ...
 <?php $query = "SELECT * FROM j_certificate"; //You don't need a ; like you do in SQL
                                    $result = mysql_query($query);?>

        <div id="page-wrapper">
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">Certificate Records</h1>
                </div>
                <!-- /.col-lg-12 -->
            </div>
            <!-- /.row -->
                     ...
                     <table width="100%" class="table table-striped table-bordered table-hover" id="dataTables-example">
                                <thead>
                                   ...
                                   <?php // start a table tag in the HTML
                                    echo "<tr><th>#</th><th>Certificate</th><th>Issued</th><th>Last Annual </th><th>Expires</th><th>Edit</th><th>Delete</th><th>View</th></tr>";
                                    echo "</thead>";
                                    echo "<tbody>";
                                    ...
                                    while($row = mysql_fetch_array($result)){   //Creates a loop to loop through results
                                         ...
                                         echo "<tr><td style='text-align: center;'>".$tmptCount."</td><td>" . $row['cert'] . "</td><td>" . $row['issued']. "</td><td>" . $row['annual'] . "</td><td>".$row['exp']."</td>";
                                        ...
                                        echo "<td style='text-align: center;'><form action='doc2.php?id=".$row['id']."' method='post' target='_blank'>
                                             <input type='hidden' name='edit_id' value='".$row['id']."' />
                                             <button class='btn btn-primary btn-xs'><i class='fa fa-download' input type='submit'></i></button></form></td></tr>";
                                             $tmptCount ++;  
